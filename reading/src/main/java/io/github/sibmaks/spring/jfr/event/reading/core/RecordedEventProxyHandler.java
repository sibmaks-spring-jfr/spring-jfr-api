...
public class RecordedEventProxyHandler<T> implements InvocationHandler {
    ...
    public RecordedEventProxyHandler(
            Map<Method, String> methods2Name,
            RecordedEvent event,
            Class<T> type
    ) {
        ...
        this.event = event;
        this.type = type;
        this.existingMethods = buildExistingMethods(event);
    }

    private static Map<String, List<Method>> buildExistingMethods(RecordedEvent event) {
        return ReflectionUtils.getMethods(event.getClass())
                .stream()
